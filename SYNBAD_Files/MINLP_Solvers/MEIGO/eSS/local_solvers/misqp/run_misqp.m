function [x,f,nfunc,ifail,g]=run_misqp(ncont,nint,nbin,m,xl,xu,x0,acc,FUN,GRAD,neq,fobj,varargin)
...
[ df,dg,nfunc ] = evaluate_grad( x,f,g,ncont,nfunc,m,df,dg,FUN,neq,fobj,GRAD,varargin{:});
ngrad = ngrad+1;
...
while(ifail <= 0) %&& count<1e3
    ...
    %if ~isempty(find(isnan(x_ls))) || ~isempty(find(isinf(x_ls)))
    %    x_ls=x_ls_or;
    %    f_ls=f_ls_or;
    %    g_ls=g_ls_or;
    %    break
    ...
    if (ifail == -2)
        % Calculate gradients
        ngrad=ngrad+1;
        [df,dg,nfunc]=evaluate_grad( x_ls(1:n),f_ls,g_ls(1:m),ncont,nfunc,m,df,dg,FUN,neq,fobj,GRAD,varargin{:});
